...
export default function QueryTabClient(initialTicker: {initialTicker: string}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
        console.log(initialTicker["initialTicker"])

            setIsLoading(true);
            try {
                const response = await fetch(`/api/v1/query?tickerName=${initialTicker["initialTicker"]}`);
                if (!response.ok) {
  ...
  const handleSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/v1/query?tickerName=${ticker}`);
      if (!response.ok) {
        throw new Error("Failed to fetch stock data");
      }
      const data = await response.json();
